...
- fix: Wait for the old terminal's exit notification before starting a replacement shell, so delayed Windows exit events cannot disable a new agent session.
...
- fix: Use short temporary launch scripts for automatic startup to avoid truncating long Codex commands. Preserve Codex hook arguments through Windows PowerShell and npm shims. Ignore stale terminal exit/output events after retry.
...
## 0.14.2 – 2026-08-23
- fix: Workspace groups now read as sections instead of looking like another small workspace. The coloured rail remains the single group-colour marker, while the redundant dot is gone; a disclosure chevron, a stronger group name and a compact neutral count establish a clearer hierarchy. Running activity marks the group heading itself, including a restrained travelling line for the spin style, so collapsed groups still preserve the information that work is happening without bringing the duplicate colour dot back
- fix: Opening a workspace with ready terminals no longer makes you guess which pane finished. The green count in the workspace register still disappears while that workspace is visible, but its information now hands off to a clear, quiet marker on each finished pane: a green-ringed status dot, a subtle inner edge and a lightly tinted header. The marker stays until you type in that pane. The new running indicator also no longer restarts its rotation on every key press; input while a pane is working now extends the same silence window without briefly changing the pane to idle, so motion remains continuous and stops only after the configured activity timeout

## 0.14.1 – 2026-08-23
- fix: Intel Macs get a build again. The macOS targets were declared without naming an architecture, so electron-builder built for whatever the machine it ran on happened to be — and that machine is an Apple Silicon runner. Every release checked back through 0.13.0 therefore carried arm64 artefacts only: an Intel Mac found nothing it could run, neither an installer to download nor an entry in the file the updater reads, and simply kept reporting that it was up to date. Both architectures are now named explicitly, so a release carries an arm64 and an x64 `.dmg` and `.zip`. Nothing changes for Apple Silicon. The step that notarises and staples the disk image already worked through every image belonging to the version being built, so the second one is covered without further change; a macOS build now takes about twice as long, because each image is submitted to Apple separately

## 0.14.0 – 2026-08-22
- feat: A workspace register can show that something is still working in it. With a dozen registers open there was no way to tell an agent that is still thinking from one that has been waiting for you for an hour — the existing "ready" badge answers the opposite question, what has finished. The colour dot of a register now marks itself while one of its panes is producing output, and the mark clears reliably the moment the shell reports it is back at its prompt, which is something output alone cannot know. Off by default; the style (quiet ring, pulse, blink, spin), its colour and its speed are yours to pick, and with reduced motion enabled system-wide every style falls back to the quiet ring. Collapsed groups show it on their chip, and remote registers show it on their server icon, so the information never hides. Two limits, both measured against real Claude and Codex sessions: a command that stays completely silent for several seconds counts as finished, and a program that repaints its display regularly can flash briefly while it waits for input
- feat: Workspace registers can be gathered into named groups. Past half a dozen workspaces the bar is a wall of similar-looking names, and the only structure available was their order — which says nothing about what belongs with what. Dragging one register onto the middle of another now puts the two into a group: it takes the colour of the register you dropped onto, appears unnamed with its name field already open, and holds its members together under a coloured rail. The outer thirds of a register keep meaning what they always did, so reordering is unchanged; only the middle groups. Dropping onto a member joins that group, dropping onto the group's chip appends to it, and dropping past its first or last member takes a register back out. A group's chip collapses it to a single entry, which hides its registers from the bar without touching the workspaces themselves — their terminals keep running, and if the workspace you are in is inside a collapsed group, the chip says so and names it rather than leaving you with nothing highlighted. Right-clicking the chip renames or dissolves the group; the command palette offers the same, plus joining and leaving, for anyone working without the mouse. Groups survive a restart. `Mod+1`…`Mod+9` still count registers in the order they appear
- fix: Typed input no longer lands in the middle of the prompt after a restart. On Windows PowerShell, starting the app with a saved maximized window created the terminals at the window's smaller stored size and only widened them a moment later — and PowerShell 5.1's line editor recomputes the column it draws from on every console resize, in a way that stays wrong once the terminal has been narrower than the prompt is long. Typing `claude` in a workspace with a long path then produced `PS C:\claudeUsers\…`, with the command itself still running correctly and only its display misplaced. The window is now sized before the terminals are created, so that resize no longer happens. The same state can be reached by hand — dragging a splitter narrow and then wide again — so a pane that is sitting at an empty prompt now repairs itself after it widens, and only then: while a program is running, nothing is sent
- change: On Windows, DM Workspace now starts PowerShell 7 (`pwsh`) when it is installed, and Windows PowerShell 5.1 otherwise. 5.1 is tied to the line editor whose cursor bug is worked around elsewhere in this release; 7 ships a version in which the cause is gone. Nothing changes on a machine that does not have PowerShell 7 installed. Worth knowing if you do: the two do not share a profile — `pwsh` reads `Documents\PowerShell\`, not `Documents\WindowsPowerShell\`
