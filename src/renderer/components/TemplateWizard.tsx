...
export function TemplateWizard(): React.JSX.Element | null {
  ...
  const STEPS = [t('template.stepNameFolder'), t('template.stepPanes'), t('template.stepReview')];
  ...
  const editing = wizard.templateId ? templates.find((tpl) => tpl.id === wizard.templateId) : null;
  ...
  useEffect(() => {
    ...
    if (editing) {
      ...
      setName(t('template.defaultName', { name: activeWs.name }));
      setCwd(activeWs.cwd);
    ...
    return (
      <div className="modal-backdrop" onMouseDown={close}>
        <div className="modal" onMouseDown={(e) => e.stopPropagation()}>
          <div className="modal-header"><span>{t('template.saveTitle')}</span>
            <button className="modal-close" title={t('common.close')} onClick={close}><Icon name="close" size={16} /></button></div>
          <p className="modal-hint">{t('template.openLayoutFirst')}</p>
          <div className="confirm-actions"><button className="confirm-btn primary" onClick={close}>{t('template.ok')}</button></div>
        </div>
      </div>
    );
  }
    ...
    <div className="modal-backdrop" onMouseDown={close}>
      <div className="modal wizard" onMouseDown={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <span>{editing ? t('template.editTitle') : t('template.saveWorkspaceTitle')}</span>
        ...
        <div className="wizard-body" ref={bodyRef}>
          {step === 0 && (
            <div className="wizard-pane-grid-2">
              <div>
                <label className="wizard-label">{t('template.nameLabel')}</label>
                <input className="wizard-input" value={name} autoFocus
                       onChange={(e) => setName(e.target.value)} placeholder={t('template.namePlaceholder')} />
                <label className="wizard-label">{t('template.workingDirectory')}</label>
                <div className="wizard-cwd">
                  <code className="wizard-cwd-value">{cwd || '—'}</code>
                  <button className="cwd-btn" onClick={() => void chooseDir()}>{t('template.choose')}</button>
                </div>
              </div>
              <div className="wizard-preview">
                <span className="wizard-label">{t('template.capturedLayout')}</span>
                <div className="mini-frame"><LayoutMini node={layout} /></div>
                <span className="wizard-preview-count">{t('template.paneCount', { count: paneIds.length })}</span>
              </div>
            </div>
          )}
              ...
              {paneIds.map((pid, i) => (
                <div className="wizard-pane-card" key={pid}>
                  <div className="wizard-pane-head">
                    ...
                    <span className="wizard-pane-index">{t('template.pane', { index: i + 1 })}</span>
                  </div>
                  <div className="wizard-pane-fields">
                    <input className="wizard-input" placeholder={t('template.paneTitlePlaceholder')}
                           value={titles[pid] ?? ''} onChange={(e) => setTitles({ ...titles, [pid]: e.target.value })} />
                    <input className="wizard-input mono" placeholder={t('template.startupCommandPlaceholder')}
          ...
          {step === 2 && (
            <div className="wizard-review">
              <div className="wizard-review-row"><span>{t('template.reviewName')}</span><strong>{name || '—'}</strong></div>
              <div className="wizard-review-row"><span>{t('template.reviewFolder')}</span><code>{cwd || '—'}</code></div>
              <div className="wizard-review-row top"><span>{t('template.reviewPanes')}</span>
                <div className="wizard-review-panes">
                  {paneIds.map((pid, i) => (
                    <div className="wizard-review-pane" key={pid}>
                      <span className="wizard-pane-index">{t('template.pane', { index: i + 1 })}</span>
                      <span>{titles[pid]?.trim() || <em>{t('template.untitled')}</em>}</span>
              ...
              {hasCommands && (
                <label className="wizard-confirm-toggle">
                  ...
                  {t('template.confirmStartupToggle')}
                </label>
              )}
            </div>
          )}
        ...
        <div className="wizard-footer">
          ...
          {step < STEPS.length - 1 ? (
            <button className="confirm-btn primary" disabled={step === 0 && !nameValid} onClick={() => setStep(step + 1)}>{t('template.next')}</button>
          ) : (
            <button className="confirm-btn primary" disabled={!nameValid} onClick={save}>
              {editing ? t('template.saveChanges') : t('template.saveTemplate')}
            </button>
          )}
        </div>
      </div>
